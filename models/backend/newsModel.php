<?php 
	trait newsModel{
		...
		public function model_do_edit(){
			...
			$conn = Connection::getInstance();
			//--------
			$result = $conn->prepare("update tbl_news set name=:name,description=:description,content=:content,hotnews=:hotnews where news_id=:id");
			$result->execute(array("name"=>$name,"description"=>$description,"content"=>$content, "hotnews"=>$hotnews ,"id"=>$id));
			//----------
			//neu user chon anh
			...
			*/
			if($_FILES["img"]["name"] != ""){
				//--------
				//lay anh cu de xoa
				$old_img = $conn->prepare("select img from tbl_news where news_id=:id");
				...
				$result = $conn->prepare("update tbl_news set img=:img where news_id=:id");
		...
		public function model_do_add(){
			...
			$conn = Connection::getInstance();
			//chuan bi cau truy van
			$result = $conn->prepare("insert into tbl_news set name=:name,description=:description,content=:content,hotnews=:hotnews,img=:img");
			$result->execute(array("name"=>$name,"description"=>$description,"content"=>$content, "hotnews"=>$hotnews ,"img"=>$img));
		}
